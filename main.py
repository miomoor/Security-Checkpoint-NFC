...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        button3 = QPushButton('Выполнить проход')
        ...
        button3.clicked.connect(self.actPassage)
        self.show()
...
class actPassage(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Выполнить проход')
        self.resize(270, 160)
        ...
        self.middle_name_text = QLabel("<center>или</center>")
        self.h5_layout.addWidget(self.middle_name_text)

        self.h6_layout = QHBoxLayout()
        self.id_card_text = QLabel("ID карты")
        self.id_card_input = QLineEdit()
        self.id_card_input.setMaxLength(8)
        self.id_card_input.setInputMask('99999999')
        self.h6_layout.addWidget(self.id_card_text)
        self.h6_layout.addWidget(self.id_card_input)

        self.h4_layout = QHBoxLayout()
        self.button_passage = QPushButton('Вход/Выход')
        self.h4_layout.addWidget(self.button_passage)
        ...
        central_widget.setLayout(layout)
        self.button_passage.clicked.connect(self.execPassage)
        self.show()

    def execPassage(self):
        uid = self.id_card_input.text()
        fn = self.first_name_input.text()
        ln = self.last_name_input.text()
        mn = self.middle_name_input.text()
        passage = QSqlQuery()
        passage.exec("select * from employees where card_id = '" + uid + "' or (first_name = '"+fn+"' and last_name = '"+ln+"' and middle_name = '"+mn+"')")
        print(passage.lastError().text())
        is_card = 0
        while passage.next():
            time_exp = datetime.strptime(passage.value(11), "%d.%m.%Y %H:%M:%S").timestamp()
            if time_exp < int(time.time()):
                self.errwin = notifyWindow("Ошибка",f'Срок действия карты истёк. Срок действия был до {passage.value(11)}. Обратитесь к дежурному охраннику.')
                self.errwin.show()
            else:
                query_is_checkin = QSqlQuery()
                query_is_checkin.exec("select card_id from history_passages where (card_id = '" + uid + "' or (first_name = '"+fn+"' and last_name = '"+ln+"' and middle_name = '"+mn+"')) and check_out = ''")
                ...
                if is_check_in:
                    ...
                    query_insert.exec("update history_passages set check_out = '" + current_time + "' where card_id = '" + uid + "' or (first_name = '"+fn+"' and last_name = '"+ln+"' and middle_name = '"+mn+"')")
                    self.errwin = notifyWindow("Ошибка", f'{passage.value(1)} {passage.value(2)} {passage.value(3)} зашёл на территорию в {current_time}')
                    self.errwin.show()
                else:
                    query_insert = QSqlQuery()
                    if passage.value(5) == "":
                    ...
                    query_insert.exec("insert into history_passages (card_id, first_name, last_name, middle_name, birth_date, rank, check_in, check_out) values ('" + uid + "', '" + passage.value(1) + "', '" + passage.value(2) + "', '" + passage.value(3) + "', '" + passage.value(4) + "', '" + rank + "', '" + current_time + "', '')")
                    self.errwin = notifyWindow("Ошибка", f'{passage.value(1)} {passage.value(2)} {passage.value(3)} покинул территорию в {current_time}')
                    self.errwin.show()
            is_card += 1
        if is_card == 0:
            self.errwin = notifyWindow("Ошибка", "<span style=\"color: red;\">Пропуск не найден</span>")
            self.errwin.show()
...
class CreateEmployees(QMainWindow):
    ...
    def createPass(self):
        ...
        try:
            ...
            last_uid = random.randint(10000000, 99999999)
